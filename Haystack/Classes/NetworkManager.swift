...
let blockCreationEndpoint = "http://35.231.51.48/api/blocks/create/"
...
class NetworkManager {
    ...
    static func createBlock() {
        ...
        AF.request(blockCreationEndpoint, method: .post, parameters: parameters, encoding: JSONEncoding.default).validate().responseData { (response) in
            switch response.result {
            case .success(let data):
                print(data)
            case .failure(let error):
